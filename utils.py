...
def draw_detections(frame, detections):
    for det in detections:
        ...
        if text:
            ...
            cv2.rectangle(frame, (x, y - th - baseline - 6), (x + tw + 6, y), color, cv2.FILLED)
            brightness = 0.299 * color[2] + 0.587 * color[1] + 0.114 * color[0]
            txt_color = (0, 0, 0) if brightness > 160 else (255, 255, 255)
            cv2.putText(frame, text, (x + 3, y - baseline - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.55, txt_color, 1, cv2.LINE_AA)
    return frame
...
def draw_hud(frame, fps, detection_count):
    ...
    if config.SHOW_FPS:
        ...
        cv2.putText(frame, fps_text, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)
    obj_text = f"Objects: {detection_count}"
    cv2.putText(frame, obj_text, (w - 200, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
    voice_state = "ON" if config.VOICE_ENABLED else "OFF"
    help_text = f"[q]uit [s]creenshot [p]ause [+/-]confidence [v]oice:{voice_state}"
    cv2.putText(frame, help_text, (10, h - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (180, 180, 180), 1, cv2.LINE_AA)
    return frame
...
def draw_info_panel(frame, detections):
    ...
    cv2.putText(frame, "Detected:", (w - panel_w + 10, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1, cv2.LINE_AA)
    for i, (label, count) in enumerate(sorted(counts.items())):
        y_pos = 88 + i * 22
        cv2.putText(frame, f"  {label}: {count}", (w - panel_w + 10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1, cv2.LINE_AA)
    return frame
